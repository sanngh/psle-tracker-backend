#!/usr/bin/env bash
# Deploys the backend to a dev Cloud Run service using .env.dev.
# Run from inside the backend/ folder: bash deploy-cloud-run-dev.sh
set -euo pipefail
cd "$(dirname "$0")"

ENV_FILE=".env.dev"
if [ ! -f "$ENV_FILE" ]; then
  echo "$ENV_FILE not found next to this script. Copy your dev env values into it first." >&2
  exit 1
fi
...
SERVICE_NAME="${SERVICE_NAME:-psle-tracker-backend-dev}"
REGION="${REGION:-asia-southeast1}"
ENV_NAME="dev"
SECRET_PREFIX="${SECRET_PREFIX:-psle-tracker-backend-dev}"

# Shared defaults if .env.dev does not include them.
export ENV="${ENV:-dev}"
...
echo "==> Done. Copy the printed service URL into your frontend dev config as EXPO_PUBLIC_API_URL."
